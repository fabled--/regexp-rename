...
タグで絞り込み（フィルタ）を設定している状態で「新規作成」した場合、
作成した正規表現が一覧から見えなくなるのを防ぐため、**絞り込みに使用しているタグが初期タグとして自動で付与されます**。
...
3. **正規化 (NFKC)**: ファイル名を Unicode 正規化（NFKC）し、必要に応じて記号や空白を統一します。
   - 新規グループ作成時、先頭に自動で追加されます。
   - 正規化の具体的な統一ルール（ON/OFF）は「設定」タブで変更できます。
...
正規化（ファイル名）の統一ルールを ON/OFF できます。

- スペース（全角→半角 / 連続スペースを1つに）
- 波ダッシュ（～ に統一）
- ダッシュ（- に統一）
- 中点（・ に統一）
- 括弧（() に統一）
...
※ プレビューと実行は同じルールで動作します（「正規化 (NFKC)」ステップが有効な場合に適用）。
...
実行時のリネーム処理は Rust 側の正規表現エンジンで行われます。
そのため、**look-around（先読み/後読み）**は使用できません（例: `(?=...)`, `(?!...)`, `(?<=...)`, `(?<!...)`）。

### 置換文字列（キャプチャ参照）の注意点（重要）

置換文字列ではキャプチャ参照として `$1`, `$2` ... を使用できます。
ただし、`$1` の直後に数字などを続けて書きたい場合（例: `$1` + `0`）、`$10` のように書くと **「10番目のキャプチャ」**として解釈される場合があります。
この問題を避けるため、**`${1}` のような形式で明示する**のを推奨します（例: `${1}0`）。

※ プレビュー表示でも `${1}` 形式を利用できます。
